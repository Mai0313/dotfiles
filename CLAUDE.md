...
- `.chezmoidata/packages.yaml` — declarative OS package lists (darwin / linux, plus `work_darwin` for Mule packages and `work_linux` for corp apt packages: google3/Cider, Android, Pontis, and serial-console tooling) and a cross-platform `npm_global` list of global npm CLIs, consumed by `.chezmoitemplates/setup-body.sh` (*nix) and `.chezmoiscripts/run_onchange_after_setup.ps1.tmpl` (Windows). Adding a package: edit the YAML and run `chezmoi apply` — `run_onchange` sees the changed content and re-runs setup.
...
9. **Input method (fcitx5)** — installs fcitx5 + Chewing + the GTK/Qt frontends and writes `~/.xinputrc` (Linux only, skipped in containers). `ibus` itself stays installed on gLinux because `gnome-shell` *depends* on it; `~/.xinputrc` is what keeps it from being the active IM.
