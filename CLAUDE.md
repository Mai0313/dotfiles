...
1. **OS packages** — apt-get + VS Code + Neovim release + lazygit GitHub release on Linux; brew on darwin. Package list lives in `.chezmoidata/packages.yaml`. VS Code install path forks on `is_work`: corp Linux (gLinux) installs the `work_linux` packages, flushes the delayed-install queue with `install-delayed-packages -u`, then runs the google3 VS Code installer; everyone else adds the public Microsoft apt repo.
   ...
   **Nothing needing a LOAS cert may run before the apt work, and nothing may abort the script over one.** The corp apt repos authenticate with the *machine* certificate, so `apt-get` never needs `gcert` — only the google3 depot the VS Code installer reads does. That is why the `work_linux` install sits first. `gcert` is interactive: it drives the gnubby over `ssh-agent` and fails immediately when `SSH_AUTH_SOCK` is unset, which is the normal case under a non-interactive `chezmoi apply`, and also inside tmux (go/sk-screen-tmux). So the branch only attempts `gcert` when `[ -t 0 ]` *and* `SSH_AUTH_SOCK` are both present, and when the cert is still missing afterwards it prints a notice and skips the installer. **Do not restore a bare `gcertstatus … || gcert`**: under `set -e` that kills the whole setup — nvm, npm, uv and IBus included — on any machine whose cert happens to be stale.
