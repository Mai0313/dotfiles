...
- The shape of a big change: plan -> check the plan holds up -> branch -> write it -> draft PR -> review -> act on what the review found -> mark ready -> CI green -> merge. A small change skips all of that and just gets made. `gh-dev-flow` owns what each step involves; this line is only the order.
