...
- Open the PR as a draft and keep it that way until the review is finished, every finding fixed or answered, and every check that ran has passed. On a repo the user does not own, checks usually wait for a maintainer to approve them, and that is not a reason to hold it.
