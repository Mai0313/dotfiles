...
- All agent skills are located under `~/.agents/skills`.
    - When writing one, point at the authoritative doc or source and say how to check an answer there, instead of copying the answer in. Field lists, flags and version numbers are a second copy that starts rotting the day it is written, and whoever reads it later cannot tell which half has gone stale. Write down only what those sources do not say: the traps, the order things have to happen in, and the decisions that cost someone a debugging session.
    - Keep its `description` to one short line saying when to reach for the skill, never how it works. It is matched against the task so the right skill gets picked; the how belongs in the body.
